...
\begin{document}
	...
	\indent\indent \textit{Delivery}. Um termo relativamente novo, aqui no Brasil e no mundo, se considerarmos a história humana, mas um conceito bem antigo: a entrega de comida. A opção de receber refeições em casa ou em qualquer outro local existe há muito tempo\cite{delivery}.
	...
	Diante deste mercado tão grande e que envolve tanto dinheiro, tem-se aproveitado dos avanços tecnológicos para continuar a crescer. Dos anos 50 para cá, criaram-se diversas facilidades tecnológicas que otimizam recursos e agilizam a realização de pedidos pelos clientes. Dentre estas tecnologias está o foco deste projeto: \textit{chatterbot}.\\
	...
	\chapter{\textit{Trabalhos Correlatos)}}  
	\indent\indent Como a maioria das invenções os \textit{chatterbots} foram criados para facilitar tarefas que normalmente exigiria uma pessoa para serem realizadas.
	
	O termo \textit{chatterbot }em si foi criado por Michael Mauldin(Deryugina, 2010) criador do primeiro Verbot(Verbal-Robot), Julia.\  
	\section{ELIZA}
	...
	\indent\indent Como\ citado, anteriormente, no tópico 3.3 - Linguagem Natural -, segundo Russel e Norving, temos dois tipos de linguagens:  Linguagens Formais e as Linguagens Naturais. Dentro de um modelo de linguagem precisamente definido, temos então, o \textit{Python}.
	...
	\textit{$``$...biblioteca é uma coleção de subprogramas utilizados no desenvolvimento de software. Bibliotecas contém código e dados auxiliares, que provém serviços a programas independentes, o que permite o compartilhamento e a alteração de código e dados de forma modular. Alguns executáveis são tanto programas independentes quanto bibliotecas, mas a maioria das bibliotecas não são executáveis. Executáveis e bibliotecas fazem referências mútuas conhecidas como ligações, tarefa tipicamente realizada por um ligador. \cite{oba7}$"$ } 
	...
	$``$...um conjunto de rotinas e padrões estabelecidos por um \textit{software} para a utilização das suas funcionalidades por aplicativos que não pretendem envolver-se em detalhes da implementação do \textit{software}, mas apenas usar seus serviços.\cite{oba12}$"$ 
	...
	\indent\indent Trataremos aqui, a implementação da solução em si. Abordando, pontos necessários para compreensão do processo de desenvolvimento. Vimos as principais definições presente no \textit{framework} Rasa, mas detalhes não menos importante, será necessário para uma boa compreensão.
	...
	$``$...criar seu primeiro \textit{bot}, adicionando todas as partes de um aplicativo Rasa, que verifique o nosso estado de humor atual e tente nos motivar quando estivermos tristes. Ele consultará nosso humor e, com base em nossa resposta, responderá com uma imagem engraçada ou uma mensagem.$"$ 
	...
	$``$...criaremos um \textit{bot} de pesquisa de restaurantes, treinando uma rede neural a partir de exemplos de conversas. Um usuário pode perguntar algum tipo restaurante - por exemplo, um restaurante mexicano -, e o \textit{bot} perguntará mais detalhes até que esteja pronto para lhe sugerir um restaurante.$"$ 
	...
	\textit{$``$...a Distribuição Anaconda é open source, e também, a maneira mais fácil e rápida, usada para fazer ciência de dados e aprendizado de máquina em Python e R, tanto no Linux, Windows ou Mac OS X. É o padrão da indústria para desenvolvimento, teste e treinamento em única máquina \cite{teste5}.$"$ }
	...
	\begin{figure}[htb]
		\begin{center}
			\includegraphics[width=6.27in,height=2.68in]{./media/teste2.png}
		\end{center}
		\caption{Modelo de um arquivo de domínio}
	\end{figure}
	...
	\begin{figure}[htb]
		\begin{center}
			\includegraphics[width=4.26in,height=4.75in]{./media/teste3.png}
		\end{center}
		\caption{Exemplo de história de conversa gerada pelo treinamento \textit{online}.}
		\label{treinamento}
	\end{figure}
	...
	Percebe-se na Fig. \ref{treinamento} que dentro do contexto de uma história, abaixo de cada \textit{intent}, temos uma ação do \textit{bot}, ou seja, uma resposta utilizando as \textit{utters}.
	...
	\begin{figure}[htb]
		\begin{center}
			\includegraphics[width=6.27in,height=4.74in]{./media/teste4.png}
		\end{center}
		\caption{Trecho do arquivo de \textit{dataset} \textit{training\_data.json}}
	\end{figure}
	...
	\begin{figure}[htb]
		\begin{center}
			\includegraphics[width=3.59in,height=1.92in]{./media/teste4.png}
		\end{center}
		\caption{Exemplo de código no \textit{Chatito}}
		\label{chatito}
	\end{figure}
	...
	Com essa simples descrição da Fig. \ref{chatito}, é possível gerar o arquivo utilizado pelo \textit{Rasa} (abaixo), e nele pode-se ver que ele gerou o \textit{Common Examples }da forma esperada.
	
	%%%%%%%%%%%%%%%%%%%% Figure/Image No: 6 starts here %%%%%%%%%%%%%%%%%%%%
	
	\begin{figure}[htb]
		\begin{center}
			\includegraphics[width=6.27in,height=2.53in]{./media/teste5.png}
		\end{center}
		\caption{Resultado da conversão do código \textit{Chatito} da Fig. \ref{chatito}.}
	\end{figure}
	...
	\begin{figure}[htb]
		\begin{center}
			\includegraphics[width=4.52in,height=1.59in]{./media/teste6.png}
		\end{center}
		\caption{Adicionado o sinônimo cardápio.}
		\label{fig:sinomimo}
	\end{figure}
	...
	\begin{figure}[htb]
		\begin{center}
			\includegraphics[width=6.14in,height=3.6in]{./media/teste7.png}
		\end{center}
		\caption{Resultado da conversão do código \textit{Chatito} da Fig. \ref{fig:sinomimo}.}
	\end{figure}
	...
	\begin{figure}[htb]
		\begin{center}
			\includegraphics[width=5.81in,height=4.32in]{./media/teste8.png}
		\end{center}
		\caption{\textit{Script} de treinamento do NLU codificado em \textit{Python}.}
	\end{figure}
	...
	\begin{figure}[htb]
		\begin{center}
			\includegraphics[width=6.27in,height=4.57in]{./media/teste10.png}
		\end{center}
		\caption{\textit{Script} de integração \textit{Rasa Core} e \textit{Rasa NLU.}}
	\end{figure}
	...
	\begin{thebibliography}{99}
		...
		\bibitem{delivery}{\em História do Delivery: uma prática antiga com novo visual}, {\url{https://blog.sistemavitto.com.br/historia-do-delivery-no-mundo/}}
		...
		\bibitem{pizzatel}Xavier, Maurício. {\em A moda de pedir pizza pelo telefone}, 1 Julho 2015, {\url{https://vejasp.abril.com.br/blog/30-anos/a-moda-de-pedir-pizza-pelo-telefone/}} 
		\bibitem{10bi} {\em Mercado de delivery de alimentos fatura mais de R\$ 10 bi no Brasil}, 27 Fevereiro 2018, {\url{http://www.abrasel.com.br/component/content/article/7-noticias/5905-27022018-mercado-delivery-de-alimentos-fatura-mais-de-r-10-bi-no-brasil.html}}
		\bibitem{foodSpain}{\em Fast food consumption in Spain will rise by 50\% over the next five years}, Lunes, 11 de Enero, 2016, {\url{https://www.eae.es/actualidad/noticias/fast-food-consumption-in-spain-will-rise-by-50-over-the-next-five-years}}
		...
		\bibitem{turing}A. M. TURING. {\em I.?COMPUTING MACHINERY AND INTELLIGENCE}, Mind, Volume LIX, Issue 236, 1 October 1950, Pages 433?460, {\url{https://doi.org/10.1093/mind/LIX.236.433}}
		\bibitem{ganascia}GANASCIA, Jean-Gabriel. {\em Inteligência artificial}. São Paulo: Editora Ática, 1997.
		\bibitem{Deryugina}Deryugina, O.V. {\em Sci. Tech.Inf}. Proc. (2010) 37: 143. {\url{https://doi.org/10.3103/S0147688210020097}}
		...
		\bibitem{rasanlu}Nichol, Alan. {\em rasa\_nlu Documentation}. Mar 23, 2018, {\url{https://media.readthedocs.org/pdf/rasa-nlu/latest/rasa-nlu.pdf}}
		...
		\bibitem{Copestake} Copestake, Ann, {\em 8 Lectures}. 2004 https://www.cl.cam.ac.uk/teaching/2002/NatLangProc/revised.pdf
		...
		\bibitem{iFood}Lima, Mariana, {\em IFOOD LANÇA CHATBOT PARA PEDIR PIZZA NO MESSENGER, MAS SERÁ SE FUNCIONA}, NoVarejo 2017. {\url{http://www.portalnovarejo.com.br/2017/07/12/ifood-chatbot-messenger/}}
		\bibitem{NLU}Navigli, Roberto. {\em Natural Language Understanding: Instructions for (Present and Future) Use}, 2018.
		\bibitem{oba}Sauvé, Jacques.  {\em Frameworks O que é um framework?}. {url{http://www.dsc.ufcg.edu.br/\~jacques/cursos/map/html/frame/oque.htm}}
		\bibitem{oba1}{\em Debian Social Contract}, Debian Project. {\url{https://www.debian.org/social\_contract\#guidelines}}
		\bibitem{oba2}{\em History of OSI}, Open Source Initiave. {\url{https://opensource.org/history}}
		\bibitem{oba3}Pereira, Fernando;  Marinho, Ivo; Oliveira, Nelson. {\em Open Source software development}.
		\bibitem{oba4}{\em Open Source Definition}, Open Source Initiave. {\url{https://opensource.org/docs/osd}}
		\bibitem{oba5}Romano, Fabrizio. {\em Learning Python}, Packt Publishing, 2015.
		\bibitem{oba6}Brennan, Bobby (2017). {\em Libraries vs. Frameworks}. {\url{https://medium.com/datafire-io/libraries-vs-frameworks-626cdde799a7}}
		\bibitem{oba7}{\em BIBLIOTECA (COMPUTAÇÃO).} In: WIKIPÉDIA, a enciclopédia livre. Flórida: Wikimedia Foundation, 2018. Disponível em: {\url{https://pt.wikipedia.org/w/index.php?title=Biblioteca\_(computa\%C3\%A7\%C3\%A3o)\&oldid=51936326}}. 
		\bibitem{oba8}{\em spaCy 101: Everything you need to know.} In: spaCy Usage Documentation. {\url{https://spacy.io/usage/spacy-101}}
		\bibitem{oba9}{\em Processing Pipeline}. In: Rasa NLU Documentation. {\url{https://nlu.rasa.com/pipeline.html}}
		\bibitem{oba10}{\em spaCy $-$ Industrial-Strength Natural Language Processing} {\url{https://spacy.io/}}
		\bibitem{oba11}{\em Language Understanding with Rasa NLU}. In: Rasa NLU Documentation. {\url{https://nlu.rasa.com/index.html}}
		\bibitem{oba12}{\em INTERFACE DE PROGRAMAÇÃO DE APLICAÇÕES}. In: WIKIPÉDIA, a enciclopédia livre. Flórida: Wikimedia Foundation, 2018. {\url{https://pt.wikipedia.org/w/index.php?title=Interface_de_programa\%C3\%A7\%C3\%A3o_de_aplica\%C3\%A7\%C3\%B5es\&oldid=52559842}}
		\bibitem{oba13}{\em	Connecting to messaging \& voice platforms}. In Rasa Core documentation. {\url{https://core.rasa.com/connectors.html}}
		\bibitem{teste1}{\em Building a Simple Bot} In: Rasa Core Documentation. \url{https://core.rasa.com/tutorial_basics.html}
		\bibitem{teste2}{\em Supervised Learning Tutorial}. In: Rasa Core Documentation. \url{https://core.rasa.com/tutorial_supervised.html}
		\bibitem{teste3}{\em Interactive Learning}. In: Rasa Core Documentation. \url{https://core.rasa.com/tutorial_interactive_learning.html}
		\bibitem{teste4}{\em Rasa Core without Python}. In: Rasa Core Documentation. \url{https://core.rasa.com/tutorial_remote.html}
		\bibitem{teste5}{\em Anaconda. The Most Popular Python Data Science Platform}. \url{https://www.anaconda.com/what-is-anaconda/}
		\bibitem{teste6}{\em PyPI - The Python Package Index}. \url{https://pypi.org/}
		\bibitem{teste7}{\em User Guide - pip Documentation}. \url{https://pip.pypa.io/en/stable/user_guide/}
	\end{thebibliography}
