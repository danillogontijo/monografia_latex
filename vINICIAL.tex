...
\begin{document}
	...
	\data{27 de Julho de 2018} % data da aprovação ??
	\cidade{Goiânia}
	%
...
%	\end{dedicatoria}
	...
	Diante deste mercado tão grande e que envolve tanto dinheiro, tem-se aproveitado dos avanços tecnológicos para continuar a crescer. Dos anos 50 para cá, criaram-se diversas facilidades tecnológicas que otimizam recursos e agilizam a realização de pedidos pelos clientes. Em 1966 surgiu o primeiro \textit{chatterbot}, ELIZA (Weizenbaum, 1966). E de maneira mais contemporânea e com o advento da internet encontramos \textit{chatterbots} sendo usados no serviço de \textit{delivery}, foco deste projeto.
	
	
	
	\textit{Chatterbot }nada mais é que um assistente virtual que irá simular conversas humanas. E com o surgimento e a popularização dos aplicativos de mensageria, como, por exemplo, \textit{Facebook Messenger}, \textit{Whatsapp}, \textit{Telegram }e outros, vamos propor a construção de um \textit{chatterbot }para auxiliar em pedidos de \textit{delivery }em um restaurante, esclarecendo sobre o cardápio, os ingredientes e as formas de pagamento. Posteriormente, esse assistente virtual, poderá ser integrado com esses aplicativos, de acordo com a documentação e instruções específicas de cada um. Além de \textit{chatterbot}, será usado termos como \textit{chatbot} ou \textit{bot} para definir a mesma coisa.
	...
	\indent\indent O ser humano é separado de outras espécies pela sua capacidade de linguagem (Russell e Norvig, p. 990). Mesmo existindo alguns outros animais capazes de se comunicar, demonstrando alguns sinais de vocabulário, somente o ser humano pode se comunicar de forma confiável utilizando um número ilimitado de mensagens qualitativamente diferentes sobre qualquer tema utilizando sinais discretos.
	...
	\indent\indent Recordando\ que gramática é um conjunto de regras definidos em uma sentença,  análise sintática é o processo de analisar uma cadeia de palavras para descobrir a sua estrutura frasal, de acordo com as regras de uma gramática (Russell e Norvig, 2013, p. 1028).
	...
	Na década de 50 os avanços na área de teoria sintática deixou as pessoas entusiasmadas e acreditava-se que em alguns anos existiriam sistemas automáticos de tradução que seriam indistinguíveis de uma tradução feita por um ser humano. Isso se provou não só irreal com o conhecimento linguístico da época  como também uma tarefa extremamente complexa.(Liddy, 2001)
	...
	\indent\indent Segundo Rich e Knight (1994), numa definição geral, Inteligência artificial (IA) é o estudo de como fazer os computadores fazerem coisas que, no momento, as pessoas fazem melhor. A IA busca analogias com a própria inteligência natural para funcionar, ou pelo menos, entender sobre IA vai ajudar também a compreender a inteligência natural.
	...
	JULIA atuava como um personagem cujo propósito era o de auxiliar outros usuários em um ambiente virtual, conhecido como \textit{TinyMUD (Multi-User Dungeons)}. JULIA ajudava usuários nesse mundo virtual mapeando cavernas e enviando mensagens.
	...
	Resumimos então, que um \textit{framework }abstrai\ algumas complexidades e códigos que são comuns a um determinado problema, com a principal característica de conter técnicas de reutilização. Facilitando, portanto, o  desenvolvimento de uma solução que se encaixa dentro do domínio de atuação do \textit{framework}. 
	...
	Compreendendo\ o conceito de \textit{framework}, as vantagens em utilizá-lo, portanto, ficam bem claras: redução de tempo e custos, a partir da maximização de seu re-uso. Detalhando mais das suas vantagens, quem está usando o \textit{framework}, se concentra em adicionar valor em vez de $``$reinventar a roda$"$ . Já as suas desvantagens, estariam basicamente na sua construção. Desenvolver um \textit{framework} é bastante complicado, o que requer um bom planejamento. Demandando, portanto,  muito tempo e esforço. Como não é o objetivo deste projeto desenvolver um \textit{framework}, mas sim apenas utilizar um pronto. Vamos ficar, então, somente com as suas vantagens.
	...
	\indent\indent Como\ citado, anteriormente em Linguagem Natural, segundo Russell e Norvig, temos dois tipos de linguagens:  Linguagens Formais e as Linguagens Naturais. Dentro de um modelo de linguagem precisamente definido, temos então, o \textit{Python}.
	...
	\begin{itemize}
		...
		\item \textbf{Marcação de parte da fala e Análise de Dependência}. Com a tokenização, o spaCy consegue atribuir tipos de palavras a cada \textit{token }gerado, como verbo ou substantivo, ou seja, é possível fazer uma análise sintática e descrever as relações entre os tokens individuais. Veja figura \ref{fig:tokenizao}. 
	...
	A intenção, mencionada na definição do Rasa NLU, é basicamente a compreensão do texto. E entidades são os nomes do mundo real, as quais falamos nas características do spaCy, os nomes de pessoas, países, produtos, etc. E por aí, já temos a ideia de onde a biblioteca spaCy será usada. Faltando, apenas dizer, como será usada. Essa parte será estudo no tópico de implementação do \textit{chatterbot}.
	
	\subsection{Rasa Core}
	\indent\indent Com\ o Rasa NLU vamos extrair as intenções das sentenças de entrada - por exemplo, se a pessoa que está conversando com o assistente virtual, está querendo saber sobre um item específico do cardápio ou se ele está apenas cumprimentando -  e também, as entidades. Mas só isso, não é suficiente para funcionar um \textit{chatterbot}. Precisamos de algo que trate e controle o fluxo da conversa, respondendo e interagindo diante dessas sentenças de entrada. Quando alguém cumprimentar o \textit{chatterbot}, que ele responda de volta um cumprimento ou boas-vindas, e se for perguntado sobre um item do cardápio, que retorne também uma resposta que esteja de acordo. É para isso então, que temos o Rasa Core. Mas não é somente isso que a ferramenta faz, temos outras funcionalidade que citaremos a seguir.
	...
	No Rasa Core, existe uma API HTTP\textit{(Hypertext Transfer Protocol)} que irá realizar exatamente essa função de abstração\cite{oba13}. E o usuário desta ferramenta, caso não saiba codificar em \textit{Python}, isso não será será um obstáculo para a desenvolver um \textit{chatterbot}. A menção desta API foi apenas para mostrar uma funcionalidade de desenvolvimento do \textit{framework}, mas não será utilizada no experimento deste projeto.
	...
	\indent\indent O \textit{Rasa} constrói histórias baseadas em exemplos. Estes exemplos são conversas completas que são utilizadas para treinar o \textit{chatterbot}. A partir dessa conversa, um fluxo de decisão do \textit{bot} é criado. Tornando assim, capaz de definir qual ação deve ser tomada baseada na entrada do usuário. 
	
	Uma opção é obter dados de conversas reais que contemplem o domínio de atuação do \textit{bot}. Porém, essa opção nem sempre é possível, pois a quantidade de informação pode não ser suficiente ou realmente não existe. Para esses casos, o \textit{Rasa} sugere como uma segunda opção o uso do chamado treinamento \textit{online}. Esse treinamento consiste em conversar com o \textit{chatterbot}, gerando um fluxo de aprendizado supervisionado, prevendo possíveis conversas entre o interlocotor e o \textit{bot}. A cada entrada será mostrado a \textit{intent} que foi identificada e a ação que o \textit{bot} acha que deve fazer, sendo possível corrigir tanto a ação quanto a \textit{intent} quando necessário. A cada erro o \textit{bot} será treinado levando em conta o novo caminho que foi informado. Isso faz com que ele possa estar sempre evoluindo, pois através desse aprendizado supervisionado é fácil corrigir comportamentos inesperados e melhorar a confiança das respostas.
	...
	* forma_pagamento
	...
	Todo o modelo treinado aqui, através do treinamento \textit{online}, é armazenado na pasta \textit{models/dialogue/}.\\
	
	\subsection{Modelo NLU}
	...
	\subsubsection{\textit{Regular Expression Features}}
	...
	Para isso, executaremos o seguinte \textit{script Python}, que está contido no arquivo \textit{dialogue\_model.py} de nosso projeto.
	...
	\begin{thebibliography}{99}
		...
		\bibitem{norvig}RUSSEL, S.; NORVIG, P.; {\em Artificial Inteligence: A Modern Approach.}, Prentice Hall Press, Third Edition, 2009.
